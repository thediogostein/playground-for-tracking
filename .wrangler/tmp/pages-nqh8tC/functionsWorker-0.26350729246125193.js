...
function validate(body) {
  ...
  if (!body.name || typeof body.name !== "string") {
    errors.push({ field: "name", message: "Nome \xE9 obrigat\xF3rio." });
  } else {
    ...
    if (trimmed.length > 100) {
      errors.push({ field: "name", message: "Nome deve ter no m\xE1ximo 100 caracteres." });
    }
  }
  if (!body.whatsapp || typeof body.whatsapp !== "string") {
    errors.push({ field: "whatsapp", message: "WhatsApp \xE9 obrigat\xF3rio." });
  } else {
    const digits = body.whatsapp.replace(/\D/g, "");
    if (digits.length < 12 || digits.length > 13) {
      errors.push({ field: "whatsapp", message: "WhatsApp inv\xE1lido. Use DDD + n\xFAmero." });
    }
  }
  if (!body.email || typeof body.email !== "string") {
    errors.push({ field: "email", message: "E-mail \xE9 obrigat\xF3rio." });
  } else {
    ...
    if (!emailRe.test(trimmed)) {
      errors.push({ field: "email", message: "E-mail inv\xE1lido." });
    }
  }
  if (!body.company || typeof body.company !== "string") {
    errors.push({ field: "company", message: "Empresa \xE9 obrigat\xF3ria." });
  } else {
    const trimmed = body.company.trim();
    if (trimmed.length < 1) {
      errors.push({ field: "company", message: "Nome da empresa \xE9 obrigat\xF3rio." });
    }
  ...
  if (!body.revenue || typeof body.revenue !== "string") {
    errors.push({ field: "revenue", message: "Faturamento \xE9 obrigat\xF3rio." });
  } else if (!ALLOWED_REVENUE.includes(body.revenue)) {
    errors.push({ field: "revenue", message: "Faturamento inv\xE1lido." });
  }
  return errors;
}
...
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  if (origin && !isAllowedOrigin) {
    return new Response(
      JSON.stringify({ success: false, error: "Origem n\xE3o permitida." }),
      { status: 403, headers }
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: false, error: "Corpo da requisi\xE7\xE3o inv\xE1lido." }),
      { status: 400, headers }
    );
  }
  ...
  const cloudflareToken = context.env.CLOUDFLARE_API_TOKEN;
      ...
      const emailResult = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${accountId}/email/sending/send`,
        {
          ...
          body: JSON.stringify({
            ...
            html: `
              <h2>Novo formul\xE1rio de contato</h2>
              <table>
...
// ../.wrangler/tmp/pages-nqh8tC/functionsRoutes-0.1416839505304054.mjs
var routes = [
  {
